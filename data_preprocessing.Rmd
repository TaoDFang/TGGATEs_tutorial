...
Basicaly, we will use rat in vivo liver diffrential gene expression data as training data and use the correspoding pathology terms as labels. All these data can be extracted from [TG-GATEs database](https://doi.org/10.1093/nar/gku955).  For your quickly understanding , imagine our training data as a matrix whose rows represent samples and coloumns represent features. In more details, each sample(row) represents diffrential gene expression levels in liver for a  rat at certain time point and cetain drug dose level. And each feature(column) represents expression level of a certain gene across different samples. Each sample has a pathological record for each pathology from TG-GATEs.
...
The method we used to caculate diffrential gene expression data is called [limma](https://www.ncbi.nlm.nih.gov/pubmed/16646809),which is a R package that fit a linear model to the expression data for each gene. By elaborate design matrix and contrast matrix, we can get various statistics from limma to (e.g. log folder changes and moderated t-statistics) describe diffrential gene expression data.  

Below is the code we used to obtain diffrential gene expresssion data for one drug.
...
From above, we get a "ExpressionSet" object storing diffrential gene expression data from a specific drug experiment.
...
<center><img src="images/TGGATEs_experiments.png" width="900px"></center>


### Data combination

<a name="single_dose_experiment"></a>In data combination step, we combine diffrential gene expression data(i.e. ".rds" data we stored in data preprocess step) from diffrent compounds in two types of studies. Besides, from pathology information in TG-GATEs,  we extract pathology terms for correspoding samples. Plese see the [Easy Start section ](easy_start.html) for final data format. 
