...
* go.bp.roche.symbols.gmt - This is the GMT format file  containing gene sets/pathways information from [MSigDB ](http://software.broadinstitute.org/gsea/msigdb) 
...
```
You can see we provide you with three  pathologies "Microgranuloma", Increasedmitosis and "Hypertrophy". During contest , it's up to you to  predict them sepretatly or try to predict three patholgies simultaneously (For example, by [multi-label classification](https://en.wikipedia.org/wiki/Multi-label_classification))
...
Each row in [gmt format file](https://software.broadinstitute.org/cancer/software/gsea/wiki/index.php/Data_formats#GMT:_Gene_Matrix_Transposed_file_format_.28.2A.gmt.29) is  a geneset, which is described by "gene set name", "gene set description" and  the genes in the gene set. 
